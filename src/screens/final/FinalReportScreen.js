...
export default function FinalReportScreen({ route, navigation }) {
  ...
  const statusColors = { start: ui.primaryBlue, priority: colors.danger, next: colors.warning, maintain: colors.success, future: colors.phase2 };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={[ui.blueGradStart, ui.blueGradEnd]} style={styles.hero}>
          ...
          <Text style={styles.heroScore}>{hii} / 100</Text>
            ...
            {DOMAIN_SHORT.map((d, i) => (
              <View key={i} style={styles.domainRow}>
                ...
                <View style={styles.domainBar}>
                  <View style={[styles.domainFill, { width: `${domainPercents[i]}%`, backgroundColor: domainPercents[i] >= 80 ? ui.primaryBlue : domainPercents[i] >= 60 ? colors.success : colors.warning }]} />
                </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 13, color: ui.midText, fontWeight: '700', marginBottom: 10, marginTop: 20, letterSpacing: 0.5 },
  scoreGrid:    { backgroundColor: ui.white, borderRadius: 16, flexDirection: 'row', padding: 16, justifyContent: 'space-around', marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 3 },
  scoreCell:    { alignItems: 'center' },
  scoreVal:     { fontSize: 28, fontWeight: '900', color: ui.primaryBlue },
  scoreLbl:     { fontSize: 11, color: ui.midText, textAlign: 'center', marginTop: 4 },
  scoreDivider: { width: 1, backgroundColor: ui.borderGray },
  threeScores:  { flexDirection: 'row', justifyContent: 'space-around', marginBottom: 8 },
  scoreRing:    { alignItems: 'center', gap: 6 },
  ringLabel:    { fontSize: 12, color: ui.midText, fontWeight: '700' },
  card:         { backgroundColor: ui.white, borderRadius: 16, padding: 16, marginBottom: 8, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 3 },
  ...
  guidanceCard: { backgroundColor: ui.white, borderRadius: 16, padding: 16, borderLeftWidth: 3, borderLeftColor: ui.primaryBlue, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 3 },
  guidanceEmoji:{ fontSize: 24, marginBottom: 10 },
  guidanceText: { color: ui.midText, fontSize: 13, lineHeight: 22 },
  roadmapRow:   { backgroundColor: ui.white, borderRadius: 14, padding: 12, flexDirection: 'row', gap: 12, alignItems: 'flex-start', marginBottom: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
  ...
  roadmapTitle:   { fontSize: 13, fontWeight: '700', color: ui.darkText },
  ...
  roadmapDesc:    { fontSize: 11, color: ui.midText, lineHeight: 16 },
  restartBtn:     { backgroundColor: ui.white, borderRadius: 14, padding: 16, alignItems: 'center', marginTop: 20, borderWidth: 1, borderColor: ui.borderGray },
  restartText:    { color: ui.midText, fontWeight: '700', fontSize: 14 },
});
